...
import useCustomerLogs from "src/react-query/dashboard/useCustomerLogs";
...
const CustomersLog = () => {
  ...
  const { data: customers, error, isPending } = useCustomerLogs();
  const skeleton = [1, 2];
    ...
    <div className="panel pt-7 pb-5 px-3 flex flex-col gap-3 h-[480px]">
      <div className="flex justify-between items-center pe-5">
        <div className="flex flex-col gap-4">
          ...
          {customers?.customersLogs && (
            <div className="text-2xl mx-5">
              {formatNumbers(customers?.totalAmount)}
        ...
        {customers?.customersLogs && (
          <div className="bc-primary text-white block py-1 text-xs px-2 rounded-md">
            {formatNumbers(customers?.logCount, false)} {data.peopleLogs.unit}
          </div>
        )}
      </div>
      <div className="flex-1 overflow-auto mb-3">
        {isPending ? (
          ...
          customers?.customersLogs?.length === 0 ? (
          <div className="text-center px-4">
            <NotFoundItem title={error?.message || data.customer.notFound} />
          </div>
        ) : (
          customers?.customersLogs?.map((item) => (
