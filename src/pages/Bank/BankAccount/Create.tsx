...
const CreateBankAccount = () => {
  ...
  const onSubmit = (datas: FieldValues) => {
    ...
    DynamicService({
      method: "post",
      service: `bank/account/${params.bid}`,
      payload: {
        ...parsingData,
        image_src: uploadedFile,
      },
    }).then((response: any) => {
      response.status === 201 && navigate(`/${params.bid}/bank/account`);
    });
  };
      ...
      <div className="p-6">
        <div className="flex gap-8">
          ...
          <div className="grid gap-x-4 gap-y-8 grid-cols-2 flex-1">
            {data.bankAccount.create.inputsTop.map((item) => (
              <InputsForm
                errors={errors}
                watch={watch}
                setValue={setValue}
