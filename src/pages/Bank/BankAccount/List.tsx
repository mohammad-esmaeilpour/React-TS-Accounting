...
type Props = {
  item: TSingleBankAccount;
  handleModal: Function;
  setBankAccount: Function;
};

const BankAccountList = ({ item, handleModal, setBankAccount }: Props) => {
  ...
  return (
    <div className="flex justify-between items-center px-3 py-3 rounded-2xl bc-secondary">
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex flex-col gap-1">
          <div className=" flex gap-2">
            <span className="family-bold">{item.bank_name}</span>
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          <CurrencyChangeIcon />
          {data.bankAccount.circulationsButton}
        </button>
        <Link
          to={`${item.id}`}
