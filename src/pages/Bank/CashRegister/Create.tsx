...
const CreateCashRegister = () => {
  ...
  const onSubmit = (datas: FieldValues) => {
    ...
    DynamicService({
      method: "post",
      service: `bank/cashregister/${params.bid}`,
      payload: {
        ...parsingData,
        image_src: uploadedFile,
      },
    }).then((response: any) => {
      response.status === 201 && navigate(`/${params.bid}/bank/cash-register`);
    });
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="panel">
      <div className="flex z-20 sticky top-0 bg-white w-full justify-between items-center border-b py-5 px-5 mb-7">
        <h1 className="family-regular text-xl">
          {data.cashRegister.create.title}
        </h1>
      ...
      <div className="p-6">
        <div className="flex gap-8">
          ...
          <div className="grid gap-x-4 gap-y-8 grid-cols-2 flex-1">
            {data.cashRegister.create.inputsTop.map((item) => (
              <InputsForm
                errors={errors}
                watch={watch}
                setValue={setValue}
