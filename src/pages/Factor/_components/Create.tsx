...
import TableCreateFactor from "src/pages/Factor/_components/CreateTable";
import ChevronDown from "src/components/icons/ChevronDown";
import { TCreateFactorData } from "src/types/Factor.types";
...
const CreateFactor = ({ factorKind, data }: Props) => {
  // state
  ...
  const onSubmit = (datas: FieldValues) => {
    const cleanedProductsList = productsList.map((product) => {
      const {
        ...
        tax_price,
        discount_price,
      } = product; // Pick only the keys you want
      return {
        product_id,
        count,
        warehouse_id,
